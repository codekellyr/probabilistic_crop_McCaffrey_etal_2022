...
Census #200
AlfArea<-AlfGrass*900
area<-cellStats(AlfGrass, sum, na.rm=T)
area * 0.000247105 #59.67413
AlfGrass2<-AlfGrass
AlfGrass2[AlfGrass2==0]<-NA
rbuff<-buffer(AlfGrass2, width=30, doEdge=T) #buffer by 30 m - one pixel layer
rbuff<-mask(rbuff, cultivated, inverse=T, maskvalue=1, updateValue=NA)
marea<-rbuff*900
area<-cellStats(marea, sum, na.rm=T)
area*0.000247105 #79966.84
#good, meets or exceeds Ag Census value
writeRaster(rbuff, paste0(wd4, "AlfalfaAgGrass_UDL_", co, "_Final.tif"), format="GTiff", overwrite=T)
...
Census # 1,350
RowArea<-Row*900
area<-cellStats(RowArea, sum, na.rm=T)
area * 0.000247105 #2208.155
#good - meets or exceeds Ag Census value
...
area * 0.000247105 #2.891129

#corn
cornarea<-corn*900
area<-cellStats(cornarea, sum, na.rm=T)
area * 0.000247105 #47777.23
